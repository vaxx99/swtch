...
func bname(ds, de string) (string, string) {
	db, _ := opendb(Cfg.Path, "/si2k/bdb/"+Cfg.Term+"/stat0.db", 0600)
	defer db.Close()
	var bn []string
	var fn string
...
func Show(buck, dbn string, w Redrec) hrec {
	...
	var rec Record
	var block []Record
	db, _ := opendb(Cfg.Path+"/si2k/bdb/"+Cfg.Term+"/", dbn, 600)
	defer db.Close()
	t1 := time.Now()
	ses := Redrec{}
	if w != ses {
...
func Call(w http.ResponseWriter, r *http.Request) {
	...
	if what != ses {
		...
		case rc < 5000:
			...
			slogs(a, c)
			t := template.New("call")
			t, _ = template.ParseFiles("si2k/xmp/call.tmpl")
			t.ExecuteTemplate(w, "call", tr)
		default:
			t := template.New("alrm")
			t, _ = template.ParseFiles("si2k/xmp/alrm.tmpl")
			t.ExecuteTemplate(w, "alrm", rc)
		}
	} else {
		http.Redirect(w, r, "/si2k/form", 301)
	}
}

func Home(w http.ResponseWriter, r *http.Request) {
	t := template.New("home")
	t, _ = template.ParseFiles("si2k/xmp/home.tmpl")
	ts := dbsize()
...
func Head(w http.ResponseWriter, r *http.Request) {
	t := template.New("head")
	t, _ = template.ParseFiles("si2k/xmp/head.tmpl")
	ts := dbsize()
...
func Form(w http.ResponseWriter, r *http.Request) {
	t := template.New("form")
	t, _ = template.ParseFiles("si2k/xmp/form.tmpl")
	ts := dbsize()
...
func dbsize() vrec {
	...
	var vc vect
	var ia, aa, bb string
	db, _ := opendb(Cfg.Path, "/si2k/bdb/"+Cfg.Term+"/stat0.db", 600)
	defer db.Close()
	db.View(func(tx *bolt.Tx) error {
		// Assume bucket exists and has keys
